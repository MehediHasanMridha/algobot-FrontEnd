...
const Testimonial = () => {
    ...
    const delete_data = async (id, img) => {
        await axios.delete(`${process.env.REACT_APP_BASE_URL}/manage/deletetestimonial/${id}`, {
            data: {
                img: img
            }
        }).then((res) => {
            ...
            <div className="flex flex-row">
                ...
                <div className={`${position} w-full lg:w-[80%] bg-gray-400 text-center relative`}>
                    <div className="text-2xl flex flex-row justify-center bg-[#207daf] p-5">
                        ...
                        <div className="w-[80%]">Testimonial</div>
                    </div>
                            ...
                            <tbody>
                                {currentItems && currentItems.map((value, index) => {
                                    return (<tr key={index} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                            ...
                                            {ReactHtmlParser(value.des.slice(1, 3) + ".....")}
                                            </td>
                                            <td className="py-4 px-6">
                                            {value.profession}
                                            </td>
                                            <td className="py-4 px-6">
                                            <img className='h-14 rounded-full' src={`${process.env.REACT_APP_BASE_URL}/course_pic/${value.img}`} alt="/" />
                                            </td>
                                            ...
                                            <button onClick={() => {
                                                ...
                                                if (ans === true) {
                                                    delete_data(value._id, value.img);
                                                }
