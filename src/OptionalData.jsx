...
const OptionalData = () => {
    ...
    const getData = async () => {
        await axios.get(`${process.env.REACT_APP_BASE_URL}/get/optionalData`)
            .then((res) => {
                setData(res.data);
    ...
    const update = (status, id) => {
        axios.put(`${process.env.REACT_APP_BASE_URL}/optionalData/${status}/${id}`).then((res) => {
            getData();
        }).catch((err) => {
    ...
    const delete_data = async (id, img) => {
        await axios.delete(`${process.env.REACT_APP_BASE_URL}/delete_paid/${id}`, {
            data: {
                img: img
            }
        }).then((res) => {
            ...
            <div className="flex flex-row">
                ...
                <div className={`${position} w-full lg:w-[80%] bg-gray-400 text-center relative`}>
                    <div className="text-2xl flex flex-row justify-center bg-[#207daf] p-5">
                        <div className="w-[80%]">Optional Data</div>
                    </div>
                                ...
                                {currentItems && currentItems.map((value, index) => {
                                    return (<tr key={index} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                        <th scope="row" className="py-4 px-6 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                            {ReactHtmlParser(value.welcomeData.slice(1, 4) + '......')}
                                        </th>
                                        <th scope="row" className="py-4 px-6 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                            {ReactHtmlParser(value.detail.slice(1, 4) + '......')}
                                        </th>
                                        <th scope="row" className="py-4 px-6 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                            {ReactHtmlParser(value.news.slice(1, 4) + '......')}
                                        </th>
                                        <th scope="row" className="py-4 px-6 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                            {value.status === "active" ? <button className='bg-red-700 p-2 rounded-lg' onClick={() => {
                                                update('inactive', value._id);
                                            }} >News Deactive</button> : <button className='bg-blue-700 p-2 rounded-lg' onClick={() => {
                                                update('active', value._id);
                                            }}>News Active</button>}
